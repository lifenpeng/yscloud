...
(function () {
    ...
    angular.module('bsTable', []).directive('bsTableControl', ["$rootScope", function ($rootScope) {
        ...
        $(document)
            ...
            .on('page-change.bs.table', CONTAINER_SELECTOR + ' table', function (evt, pageNumber, pageSize) {
                ...
                //yx000(2017-01-18): mark last page number in current table
                if (angular.element(evt.target).attr('bs-table-control')) {
                    $rootScope.global_data.last_page[angular.element(evt.target).attr('bs-table-control')] = {pageNumber: pageNumber};
                }
            })
        ...
        return {
            ...
            link: function ($s, $el, $attrs) {
                //yx000(2017-01-18): 清空其他表格历史记录并赋当前历史值
                for(var o in $rootScope.global_data.last_page) {
                    if(o !== $attrs.bsTableControl) $rootScope.global_data.last_page[o].pageNumber = 1;
                }
                var bsTable = bsTables[$s.$id] = {$s: $s, $el: $el};
                $s.instantiated = false;
                $s.$watch('bsTableControl.options', function (options) {
                    $s.bsTableControl.options.pageNumber = $rootScope.global_data.last_page[$attrs.bsTableControl] ? $rootScope.global_data.last_page[$attrs.bsTableControl].pageNumber : 1;
